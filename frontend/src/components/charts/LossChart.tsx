...
export function LossChart({ trainLoss, testLoss, width = 480, height = 280 }: LossChartProps) {
    ...
    <div className="chart-block">
      <div className="chart-header">
        ...
        <ul className="chart-legend">
          ...
          <li className="chart-legend-item">
            <span className="chart-legend-swatch" style={{ background: TEST_COLOR }} />
            Test
          </li>
        </ul>
      </div>
      ...
      <div
        ...
        tabIndex={0}
        role="group"
        aria-label={`Loss per epoch: training and test loss across ${epochCount} epoch${epochCount === 1 ? '' : 's'}. Use arrow keys to inspect values.`}
      >
        <svg width={width} height={height}>
          ...
          {testLoss.length > 0 && (
            <>
              <circle cx={xForIndex(testLoss.length - 1)} cy={yForValue(testLoss[testLoss.length - 1])} r={4} fill={TEST_COLOR} stroke={SURFACE} strokeWidth={2} />
              <text
                x={xForIndex(testLoss.length - 1) + 8}
                y={yForValue(testLoss[testLoss.length - 1]) + 3}
                fontSize={10}
                fill={INK_SECONDARY}
              >
                {testLoss[testLoss.length - 1].toFixed(2)}
              </text>
            </>
          )}
          ...
          {tooltip && (
            <g>
              ...
              <circle cx={tooltip.x} cy={yForValue(tooltip.test)} r={4} fill={TEST_COLOR} stroke={SURFACE} strokeWidth={2} />
            </g>
          )}
        </svg>
        ...
        {tooltip && (
          <div className="chart-tooltip" style={{ left: tooltipLeft }}>
            ...
            <div className="chart-tooltip-row">
              <span className="chart-legend-swatch" style={{ background: TEST_COLOR }} />
              <span>Test</span>
              <strong>{tooltip.test.toFixed(4)}</strong>
            </div>
          </div>
        )}
      </div>
      ...
      <table className="sr-only">
        <caption>Loss per epoch (training and test)</caption>
        <thead>
          <tr>
        ...
        <tbody>
          {Array.from({ length: epochCount }, (_, index) => (
            <tr key={`row-${index}`}>
              ...
              <td>{testLoss[index]?.toFixed(4) ?? '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
